...
export const [FitnessProvider, useFitness] = createContextHook(() => {
  ...
  const loadData = async () => {
    try {
      const [activitiesData, weightData, waterData, moodsData, badgesData, statsData, sleepData, reminderData, goalsData, plansData, sessionsData] = await Promise.all([
  ...
  const updateDailyStats = (activity: Activity) => {
    ...
    const existingStats = dailyStats.find(s => s.date === today);
    ...
    const newStats = dailyStats.filter(s => s.date !== today);
  ...
  const addWaterIntake = (amount: number) => {
    const today = new Date().toISOString().split('T')[0];
    const existing = waterIntakes.find(w => w.date === today);
    
    let updated: WaterIntake[];
    if (existing) {
      updated = waterIntakes.map(w => 
        w.date === today 
  ...
  const getActivityStreak = (): number => {
    const sortedDates = [...new Set(activities.map(a => a.date.split('T')[0]))].sort().reverse();
    let streak = 0;
  ...
  const getWaterGoalDays = (): number => {
    return waterIntakes.filter(w => w.amount >= w.goal).length;
  };

  const getTodayWaterIntake = (): WaterIntake => {
    const today = new Date().toISOString().split('T')[0];
    return waterIntakes.find(w => w.date === today) || {
      id: generateId(),
      amount: 0,
      date: today,
      goal: 8,
  ...
  const getTodayMood = (): Mood | null => {
    const today = new Date().toISOString().split('T')[0];
    return moods.find(m => m.date.split('T')[0] === today) || null;
  };
  ...
  const upsertGoal = async (partial: Omit<Goal, 'id' | 'createdAt' | 'updatedAt' | 'isActive'> & { id?: string; isActive?: boolean }) => {
    ...
    let updated: Goal[] = [];
    if (partial.id) {
      updated = goals.map(g => g.id === partial.id ? { ...g, ...partial, updatedAt: now, isActive: partial.isActive ?? g.isActive } as Goal : g);
    } else {
      const newGoal: Goal = {
        id: generateId(),
        type: partial.type,
  ...
  const deleteWorkoutPlan = async (id: string) => {
    const updated = workoutPlans.filter(plan => plan.id !== id);
    setWorkoutPlans(updated);
    await AsyncStorage.setItem(STORAGE_KEYS.WORKOUT_PLANS, JSON.stringify(updated));
  };
